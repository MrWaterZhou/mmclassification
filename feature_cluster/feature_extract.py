...
if __name__ == '__main__':
    ...
    err_images = [x.strip() for x in open(sys.argv[1])]
    target_images = [x.strip() for x in open(sys.argv[2])]

    f2 = model.get_normalized_feature(target_images)
    for x in err_images:
        result = model.get_most_similar(x, (target_images, f2))
        print(x, result)
