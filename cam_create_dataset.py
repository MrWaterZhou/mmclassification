    ...
    def run(self) -> None:
        for file in self.file_list:
            ...
            if len(label_choices)>0:
                label = np.random.choice(label_choices)
                try:
...
class Saver(Thread):
    ...
    def run(self) -> None:
        while True:
            data, image, grayscale_cam = self.save_queue.get()
            ...
            try:
                ...
                new_father_path = father_path + '_mosaic'
                ...
                self.result_queue.put(json.dumps(data, ensure_ascii=False) + '\n')
