...
class Runner:
    def __init__(self, arch, max_batch_size, engine_path, labels, image_queue: Queue, save_path: str, threshold=0.5):
        ...
        if arch == 'regnetx':
            self.BGR = True
        self.threshold_list = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9]
        ...
        self.TP = {t: {x: 0 for x in self.labels} for t in self.threshold_list}
        self.TN = {t: {x: 0 for x in self.labels} for t in self.threshold_list}
        self.FP = {t: {x: 0 for x in self.labels} for t in self.threshold_list}
        self.FN = {t: {x: 0 for x in self.labels} for t in self.threshold_list}
            ...
            for filename, res in zip(filenames, results):
                ...
                for r, label in zip(res, self.labels):
                    ...
                    for threshold in self.threshold_list:
                        if (r > threshold) and (label != '正常') and ('正常' in self.labels):
                            res[-1] = 0.0
                        if r > threshold:
                            if filename[label] == 1:
                                self.TP[threshold][label] += 1
                            else:
                                self.FP[threshold][label] += 1
                                if threshold == self.threshold:
                                    tags['误检'].append(label)
                        else:
                            if filename[label] == 1:
                                self.FN[threshold][label] += 1
                                if threshold == self.threshold:
                                    tags['漏检'].append(label)
                            else:
                                self.TN[threshold][label] += 1
...
if __name__ == "__main__":
    ...
    for l in labels:
        for threshold in runner.threshold_list:
            precision = runner.TP[threshold][l] / (runner.TP[threshold][l] + runner.FP[threshold][l] + 1e-5)
            recall = runner.TP[threshold][l] / (runner.TP[threshold][l] + runner.FN[threshold][l] + 1e-5)
            print('{}, precision@{}:{}, recall@{}:{}'.format(l, threshold, precision, threshold, recall))
    print('done')
